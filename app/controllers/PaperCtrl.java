...
public class PaperCtrl extends Controller {
    ...
    public Result get(Long id) throws ServiceException {
        return getServiceRest.get( LcPaper.class, id ).buildPlayCtrlResult();
    }

    @Transactional(readOnly = true)
    public Result query(Integer page, Integer perPage, String filters) throws ServiceException {

        JavaServiceResult jsr = getServiceJava.count(LcPaper.class, filters);
        ...
        return queryServiceRest.query(LcPaper.class, page, perPage, filters ).buildPlayCtrlResult();
    }

    @Transactional(readOnly = false)
    public Result update(Long id) throws ServiceException {
        return updateServiceRest.update( LcPaper.class, request().body().asJson(), id ).buildPlayCtrlResult();
    }

    @Transactional(readOnly = false)
    public Result create() throws ServiceException {
        return createServiceRest.create( LcPaper.class, request().body().asJson() ).buildPlayCtrlResult();
    }

    @Transactional(readOnly = false)
    public Result delete(Long id) throws ServiceException {
        return deleteServiceRest.delete( LcPaper.class, id ).buildPlayCtrlResult();
    }
}
